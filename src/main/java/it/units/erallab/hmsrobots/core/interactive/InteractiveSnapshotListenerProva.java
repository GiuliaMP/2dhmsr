...
public class InteractiveSnapshotListenerProva implements SnapshotListener {
  ...
  public InteractiveSnapshotListenerProva(double dT, CanvasManager canvasManager,
                                     DevicePoller devicePoller,
                                     PropagationController controller,
                                     int totalTime,
  ...
  public void listen(double simT, Snapshot s) {
    ...
    if (lastDrawT == 0.0d || lastDrawT + frameDT <= realT) {
      ...
      flag.add(controller.getActive());
      flagHistory.putIfAbsent(simT, flag);
    }
